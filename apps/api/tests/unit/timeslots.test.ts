...
describe("formatFreeSlotText", () => {
  it("空きスロットをメールに貼れる日本語の行にする (記号なし・曜日つき)", () => {
    // 2026-07-09 は木曜, 07-10 は金曜
    const text = formatFreeSlotText([iv(9, 10, 9, 12), { start: at(10, 14, 30), end: at(10, 17) }]);
    const lines = text.split("\n");
    expect(lines).toHaveLength(2);
    expect(lines[0]).toBe("7月9日(木) 10時から12時");
    expect(lines[1]).toBe("7月10日(金) 14時30分から17時");
    // BR-09: 箇条書き記号を出さない
    expect(text).not.toMatch(/[・*＊#※-]/);
  });
  it("空なら空文字", () => {
    expect(formatFreeSlotText([])).toBe("");
  });
});
