...
## ⚠️ 大事なお知らせ（2026-07-18 のセキュリティ修正）

これまで bonds の画面は、**ログインしなくても**開けば使えていました。ですが、それだと
「URL を知っただけの他人」にもあなたの連絡先が見えてしまう穴があったため、修正しました。

- これからは、**Google（agewaller@gmail.com）でログインしてから**使ってください。
  ログインすると、これまでどおり、あなたの連絡先・記録・管理画面がすべて見えます。
- ログインしていないと、画面は「サインインが必要です」になります（あなたのデータは無事です）。
- **もしログインしても連絡先が空に見える**ときは、ログインしたメールが agewaller@gmail.com に
  なっているか確認してください。違うメールだと別の人として扱われます。それでもだめなら、
  止めて私（Claude）に「ログインしたのに連絡先が空」と伝えてください（緊急の復旧口も用意してあります）。
...
> 一斉配信（お便り）について: 連絡帳の「一斉配信（お便り）」から、1 通の文面を選んだ相手に
> まとめて送れます。**費用**は Resend の無料枠が月3,000通・1日100通。それを超えて送るなら
> 有料（おおよそ月$20 で5万通・要確認）。全員同じ文面＋お名前差し込みなので **AI 費用はかかりません**。
> **到達性のコツ**: 1 日の上限は控えめ（100〜200通）にして少しずつ送ると、迷惑メール判定を避けられます。
> 送信先は「実際にご縁のある方」に絞ってください（面識のない多数への配信は特定電子メール法の面でも避ける）。
> 配信停止リンクと差出人は自動で付きます。ドメイン認証（上の Verify a Domain）を済ませておくほど届きやすくなります。
...
### 1-D. 「送信できませんでした」「API key is invalid」と出たときの直し方（2026-07-20 実際に発生）

- ▼ ねらい: bonds に入っている Resend の鍵が古い・打ち間違い・削除済みのとき、**新しい鍵に入れ替えて**送れるようにする。
- 症状: メールの承認→送信で失敗する。診断すると Resend が「API key is invalid（鍵が無効）」と答えている。
  鍵の「形」は合っていても、Resend 側でその鍵が生きていないと送れません（鍵を作り直した・消した後に起こります）。
- 手順（5分・上から順に）:
  1. https://resend.com にログイン →【検索キーワード: API Keys】を開く。
  2. **「Create API Key」**（鍵を作る）を押し、名前は `bonds` などにして作成。
     表示された **`re_` で始まる文字列をコピー**（この画面を閉じると二度と見られません。閉じてしまったらもう一度作ればよいです）。
  3. 上の **1-A の手順**どおり、Secret Manager の `BONDS_SENDGRID_API_KEY` に**新しいバージョン**として貼り付ける。
     （余計な空白や改行が入らないよう、値の欄に直接貼り付けてください）
  4. **本番へ反映**（私に「本番反映して」でもよいです。鍵はデプロイのタイミングで読み込まれるため、入れ替えだけでは反映されません）。
  5. 確認: 私に「メール送信の点検をして」と言えば、あなた宛にテストメールを1通送って生死を確かめられます。
- 分からなければ止めて、画面の写真を送って相談してください。アプリは鍵が直るまで「送信だけ」が失敗し、他は壊れません。

### 1-E. いちばん簡単な代わりの道：個別メールを「あなたの Gmail」から送る（おすすめ）

- ▼ ねらい: 配信サービスの鍵が無くても、一対一のメールをあなたの Gmail (agewaller@gmail.com) から送れるようにする。
- 相手には普段のあなたのメールとして届くので迷惑メールになりにくく、**相手の返信もあなたの Gmail に残ります**。
  送るのは bonds で「承認」した文面だけです。一斉配信（お便り）は引き続き配信サービス側を使います。
- 手順（1 分）:
  1. bonds の **設定ページ**（連絡帳右上の「設定」）を開く。
  2. Google 連携の欄にある **「メールを自分の Gmail から送れるようにする（Google の許可）」** を押す。
  3. Google の同意画面で許可する（「確認されていないアプリ」の警告が出たら「詳細」→「移動」で進めて大丈夫です。自分専用のアプリです）。
- これだけで、以後の個別メールは自動的に Gmail から送られます（設定欄に「Gmail から送られます」と表示されます）。

### 1-F. （上級・任意）どのメールサーバでも送れるようにする（SMTP）

- ▼ ねらい: Resend や SendGrid 以外（さくらのメール・会社のメールサーバなど）から送りたいときの道。
- Secret Manager に **`BONDS_SMTP_URL`** という名前でシークレットを作り、値に接続アドレスを入れて本番反映します。
  形式: `smtps://ユーザー名:パスワード@メールサーバ:465`（ユーザー名に @ が入るときは `%40` と書きます）。
- 設定すると Resend/SendGrid より SMTP が優先されます。通常は 1-D（Resend）か 1-E（Gmail）で足りるので、必要になったときだけで大丈夫です。
...
> 録音メモ（Plaud）のタスク整理について: 連絡帳の「録音メモからのタスクと課題」を使うには、
> このタスク2の Google 連携に加えて、**「録音メモを読めるようにする（Google の許可）」ボタンから
> もう一度だけ Google の同意画面を通す**必要があります（メールを読む追加の許可 = gmail.readonly）。
> bonds が読むのは録音サービス（plaud）から届いたメールの**添付テキストだけ**で、ほかのメールの
> 中身は使いません。同意画面に「確認されていないアプリ」の警告が出たら、「詳細」→「（アプリ名）に
> 移動」で進めます（自分で作った自分専用のアプリなので問題ありません）。
...
## タスク6：Google の「確認済みアプリ」にする（「危険なページ」警告を消す）

Google 連携のボタンを押したとき、いまは「このアプリは Google で確認されていません」という
**こわい警告画面**が出ます（「詳細」→「（安全でないページ）に移動」と押せば進めますが、
初めての方はここでやめてしまいます）。これを消すには、Google にアプリの「確認（審査）」を
申請します。**急ぎではありません**。設定しなくてもアプリは壊れず、警告つきで動き続けます。

- ▼ ねらい: ①bonds 専用の「自分の持ち物のドメイン」を用意する ②同意画面を「本番」にして
  アプリ情報をそろえる ③確認を申請する。審査は数日〜数週間かかります。
- 大事な前提: bonds は権限を二段階に分けてあります。**ふだんの連携は「連絡先とカレンダーの
  読み取り」だけ**（審査が軽い区分）で、メール・共有ファイルの読み取りは「追加の許可」を
  押した人だけに出ます（こちらは審査がとても重い区分なので、**申請に含めない**のがコツです）。

### 6-A. 自分の持ち物のドメインを bonds に付ける（いちばんの難関・私に相談推奨）
- ▼ ねらい: Google の審査は「アプリのホームページが、あなたの持ち物だと証明できる住所
  （ドメイン）にあること」を求めます。いまの `bonds-web-….run.app` は Google の貸し住所なので
  証明できません。cares で使っている **advisers.jp のような自分のドメイン**に、
  例えば `bonds.advisers.jp` を作って bonds の顔にします。
- 手順（ここは私＝Claude と一緒にやるのがおすすめ。「bonds に独自ドメインを付けたい」と
  声をかけてください。Cloud Run のドメイン設定と DNS 登録を私が案内します）:
  1. 使うドメイン名を決める（例: `bonds.advisers.jp`）。
  2. Google Search Console（https://search.google.com/search-console）でそのドメインの
     所有権を確認する（DNS に確認用の文字列を1行足すだけ。cares で advisers.jp を確認済み
     なら、そのままで大丈夫なことが多いです）。
  3. Cloud Run の「カスタムドメイン」で bonds-web にそのドメインを割り当てる。
- 画面が違うとき: ここは手順が多いので、迷ったら止めて私に「タスク6のドメインで詰まった」と
  伝えてください。画面の写真があるとすぐ分かります。

### 6-B. 同意画面を「本番」にして、アプリ情報をそろえる
- ▼ ねらい: 「テスト」状態のアプリを「本番」に切り替え、審査に必要な情報を埋める。
- 手順:
  1. https://console.cloud.google.com/apis/credentials/consent?project=arctic-anvil-497002-q2 を開く。
     （【検索キーワード: OAuth 同意画面 / OAuth consent screen / ブランディング】）
  2. 「公開ステータス」が「テスト」なら **「アプリを公開」（Publish app）** を押して本番にする。
  3. アプリ情報を埋める:
     - アプリ名: `bonds`
     - サポートメール: agewaller@gmail.com
     - **アプリのホームページ**: 6-A で付けたドメイン（例 `https://bonds.advisers.jp`）
     - **プライバシーポリシー**: `https://（同じドメイン）/privacy`
       （このページは作成済みです。bonds のトップの下にもリンクがあります）
     - 承認済みドメイン: 6-A のドメイン（例 `advisers.jp`）
  4. 保存する。
- 画面が違うとき: 新しい Console では「ブランディング」「対象」「データアクセス」に分かれて
  います。アプリ名やホームページは「ブランディング」、公開は「対象」にあります。

### 6-C. スコープ（権限）を登録して確認を申請する
- ▼ ねらい: 「このアプリはこの権限を使います」と申告して、審査ボタンを押す。
- 手順:
  1. 同じ同意画面の「データアクセス」（または「スコープ」）で「スコープを追加または削除」を押す。
  2. 次の**2つだけ**にチェックを入れて保存（フィルタに `calendar.readonly` などと入れると早い）:
     - `.../auth/calendar.readonly`（カレンダーの読み取り）
     - `.../auth/contacts.readonly`（連絡先の読み取り）
     - ※ `gmail.metadata` と `drive.metadata.readonly` は**入れない**でください。入れると
       「制限付きスコープ」の重い審査（外部のセキュリティ検査・有料）になります。bonds は
       この2つが無くても動きます（「追加の許可」ボタンだけ警告つきになりますが、それで正常です）。
  3. 「確認を申請」（Submit for verification / Prepare for verification）を押し、質問に答える:
     - 何に使うか（英語で聞かれたら）: 例
       `bonds imports the user's own Google Contacts and calendar attendees, read-only, to build the user's personal address book.`
     - デモ動画を求められたら: 止めて私に「審査でデモ動画を求められた」と伝えてください。
       画面の録画を一緒に用意します。
  4. 申請後は Google からメールが来ます（数日〜数週間）。追加の質問が来たら、そのまま私に
     転送してもらえれば返事の文面を作ります。
- 確認: 審査が通ると、Google 連携のボタンを押したときの「確認されていません」警告が消え、
  ふつうの「bonds が次の許可を求めています」画面だけになります。

### いま警告が出ても大丈夫、について
- 審査が終わるまでの間も、警告画面で「詳細」→「移動」と押せば連携できます（あなた自身や
  身近な方が使う分には問題ありません）。
...
## タスク9：統合LPの「Google でログイン」を有効にする（承認済みドメインの追加・1回だけ）

▼ ねらい: 統合LP（https://agewaller.github.io/bonds/products.html）にログインボタンを出し、
ログインした方に「あなたの道具」（五つの道具への入口）を表示できるようにする。

仕組みはすでに実装済みで、残る設定はこれ1つです。終わるまでは LP でログインが
エラーになるだけで、ページそのものは壊れません。

1. https://console.firebase.google.com/ を開き、健康日記と共用しているプロジェクトを選ぶ
   【検索キーワード】Firebase Authentication 承認済みドメイン（または Authorized domains）
2. 左メニューの「Authentication（認証）」→ 上部タブ「Settings（設定）」→
   「Authorized domains（承認済みドメイン）」を開く
3. 「ドメインを追加（Add domain）」を押して、次を 1 行で入れる:
   `agewaller.github.io`
4. 保存すれば完了。デプロイは不要で、その場から LP のログインが通るようになります

- 画面が違うとき: 「Authentication」が見当たらなければ左メニューの「構築（Build）」の中にあります。
  分からなければ止めて、画面の写真を送って相談してください。

---

## タスク10：提携・紹介の連絡を「自分の Gmail から」送れるようにする

▼ ねらい: 配信サービス (Resend) 経由の提携連絡がバウンス率超過でアカウント停止を招いたため、
未面識の相手への連絡はオーナー本人の Gmail から少量ずつ送る方式に切り替えた。その許可を出す。

### 10-A. Gmail 送信の許可（1分・デプロイ不要）

1. bonds の設定ページ（/settings）→ Google 連携の欄を開く
2. 「提携の連絡を自分の Gmail から送れるようにする」ボタンを押す
3. Google の同意画面で許可する（送るのはあなたが承認した文面だけです）
- 注: この権限は Google の「制限付き」区分のため、同意画面に警告が出ることがあります
  （タスク6のアプリ確認が済むと消えます）。自分自身のアカウントに許可する分には問題ありません
- 許可が済むまで、承認済みの提携メールは「保留」のまま送られません（壊れません）

### 10-B. 宛先の事前検証を有効にする（任意・推奨・15分）

▼ ねらい: 送る前に宛先の実在を自動チェックし、無効な宛先には送らない（バウンス再発の根絶）。

1. https://www.zerobounce.net/ でアカウントを作る（無料枠 月100件。有料は従量で $16/2,000件ほど）
   【検索キーワード】ZeroBounce API key
2. 管理画面で API キーを取得する
3. Google Cloud の Secret Manager で **`BONDS_EMAIL_VERIFY_API_KEY`** を作成して値に貼る
   https://console.cloud.google.com/security/secret-manager?project=arctic-anvil-497002-q2
4. Claude に「bonds を本番反映して」と伝える
- 未設定でも送信は動きます（検証なしになるだけ）。分からなければ止めて画面の写真を送って相談してください
