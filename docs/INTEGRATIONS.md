...
| Google 連絡先 | People API はあるが OAuth 審査・検証が必要 | contacts.google.com のエクスポート (CSV/vCard) + 審査済みなら読み取り OAuth |
| Outlook / Microsoft | Graph API はあるが Azure アプリ登録・同意が必要 | Outlook の「連絡先のエクスポート」CSV / 予定表の公開 ICS |
...
- **Outlook 連携** (2026-07-10): 連絡先は Outlook の「連絡先のエクスポート」CSV を
  そのまま取り込む。Outlook の CSV は列が非常に多く列名も独特 (First Name / Last Name /
  E-mail Address / Business Phone / Mobile Phone / Job Title、日本語 UI は 姓 / 名 /
  電子メール アドレス / 勤務先 電話 / 携帯電話 / 役職) なので、汎用 CSV や LinkedIn 用に
  任せると電話・メール・役職を取りこぼす。専用の `parseOutlookContacts` / `looksLikeOutlookCsv`
  (`lib/contact-parsers.ts`) を Google の次・LinkedIn の前に自動判別で挟み、メールは 1→2→3、
  電話は携帯→勤務先→自宅の順で最初に埋まっているものを採る。誕生日はゼロ詰めなしの
  `1965/6/1` も `normalizeIsoDate` で正す。予定表は Outlook の公開 ICS アドレスを既存の
  「ご自身の予定表とつなぐ」(`PUT /api/relationship/my-busy` の icsUrl) に貼れば面談候補に
  使える (Google カレンダーの非公開 ICS と同じ経路)。連絡帳上部の連携ボタンに Outlook を追加。
- **Microsoft Graph の読み取り OAuth** は Google と同じく「審査済みなら本物の連携」枠。
  Azure アプリ登録 + 同意画面が前提のため、登録されるまではエクスポート CSV / ICS で
  同じ価値を提供する (Google OAuth クライアントが未設定のとき CSV で代替するのと同思想)。
...
**取り込んだ後の自動化 (プロフィール自動充実):** やりとり・贈り物・取込済みの記録が
増えるたび、毎時の sweep が「いまのこの方」ノート (`contacts.profile_digest`、暗号化) を
自動でまとめ直す。web 検索はユーザーが連絡先画面で明示的に押したときだけ行う
(私人を自動巡回で検索しない = 相手の尊厳)。ユーザーが自分で書いた項目は上書きしない。
...
## 4. ZenTrack (音声文字起こしの日次インサイト) → bonds の関係グラフ (2026-07-09)

ZenTrack は Plaud 等の音声文字起こしをメールで受け取り AI で日次インサイト化する
別プロダクト (Spring Boot/Java・Firebase 認証・PostgreSQL)。日々の文字起こしには
「誰と会った・誰の近況」が詰まっているので、bonds の受動収集の一次ソースになる。

- スタックが違う (Java ⇄ TypeScript) ので**コード合体ではなくデータ連携** (HTTP)。
- **受け口**: `POST /api/ingest/zentrack` (server-to-server)。ZenTrack 専用の共有
  シークレット `x-zentrack-secret: <ZENTRACK_INGEST_SECRET>` で認証 (オーナーの管理
  トークンは渡さない)。未設定なら 503 =「準備中」に fail closed。
- **本文**: `{ "transcript": "文字起こし全文", "date": "YYYY-MM-DD", "label": "任意の見出し" }`。
- **挙動**: 既存の取込ジョブ (`import_jobs`) に預け、会話抽出 (`import_extract` プロンプト)
  で人物・近況・接触日を取り出し、`applyImport` で連絡帳へ (名寄せ・冪等・暗号化は共通)。
  すぐ処理し、失敗しても毎時 sweep が拾い直す。bonds は単一オーナー運用なので owner に紐づく。
- **設定**: Secret Manager に `BONDS_ZENTRACK_INGEST_SECRET` を作成すると 07 デプロイが
  存在時のみ配線する (無ければデプロイは通り、受け口だけ準備中)。ZenTrack 側は解析完了時に
  この URL へ POST する (ZenTrack の Java 側に1本 outbound を足す = 次段)。
- **将来**: ZenTrack の意識・状態インサイトを bonds の軸②(ユーザー自身のリソース/状態)へ
  接地する経路も同様に足せる。
