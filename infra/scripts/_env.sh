...
# デバイス連携 (Oura リング / Withings マット)。未作成なら該当プロバイダは「準備中」に縮退。
export SECRET_OURA_CLIENT="${SECRET_OURA_CLIENT:-BONDS_OURA_CLIENT_SECRET}"
export SECRET_WITHINGS_CLIENT="${SECRET_WITHINGS_CLIENT:-BONDS_WITHINGS_CLIENT_SECRET}"
...
# 汎用 SMTP の接続 URL (任意。例 smtps://user:pass@mail.example.com:465。設定時は SMTP を最優先)
export SECRET_SMTP_URL="${SECRET_SMTP_URL:-BONDS_SMTP_URL}"
