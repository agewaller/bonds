...
if [ "$ONLY" != "web" ]; then
  ...
  gcloud run deploy "$RUN_API" --project="$PROJECT" --region="$REGION" \
    ...
    --set-secrets="$SECRETS" \
    --set-env-vars="ALLOWED_ORIGINS=${WEB_URL:-$PROD_WEB_URL},SQL_CONN=${SQL_CONN},SQL_DB=${SQL_DB},SQL_USER=${SQL_USER},FIREBASE_PROJECT_ID=${FIREBASE_PROJECT_ID:-$PROJECT},OWNER_EMAIL=${OWNER_EMAIL},OUTREACH_FROM_EMAIL=${OUTREACH_FROM_EMAIL:-},PARTNER_AUTO_SEND=${PARTNER_AUTO_SEND:-0},PARTNER_DAILY_LIMIT=${PARTNER_DAILY_LIMIT:-20},OUTREACH_SENDER_IDENTITY=${OUTREACH_SENDER_IDENTITY:-},GOOGLE_OAUTH_CLIENT_ID=${GOOGLE_OAUTH_CLIENT_ID},GOOGLE_OAUTH_REDIRECT_URL=${GOOGLE_OAUTH_REDIRECT_URL},PERSON_DD_MONTHLY_CAP_JPY=${PERSON_DD_MONTHLY_CAP_JPY:-0}" \
