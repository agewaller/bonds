...
- 連絡先がわからない方を、つながりでたどる（2026-07-23・オーナー依頼「登録関係者それぞれの課題（連絡先がわからない）などをできるだけ解決して（別の登録者から連絡すれば繋がるなど）」）— 実装済み: `lib/reachability.ts` の純粋関数（AI 不要・毎回無料・web 検索なし）。①`isReachable`＝メール・電話・SNS のどれも無い方を「届かない方」と判定 ②`pickUnreachableTargets`＝くり返し登場・やりとり・近い距離感で「解決する価値の高い方」から最大20名 ③`findBridges`＝連絡手段のある登録者の中から橋渡し役を決定的な手がかりで採点（同じ所属 30・同じ日の同席 25・同じイベントで出会う 25〔メモの「◯◯で出会う」を突合〕・記録への登場 20〔3文字以上の名前のみ〕）→ 上位3名。`GET /api/relationship/reachability`（橋渡し役つきの方を先に最大12名 + unreachableTotal + SNS 候補の有無）。web は連絡帳「連絡先がわからない方を、つながりでたどる」パネル（橋渡し役リンク + ✉ お願いのメール〔件名入り mailto〕+ 実行待ちに入れる + ✖️見送り kind=reachability。橋渡しが無い方には SNS 候補の案内 or 取り込みの促し）。テスト: reachability unit 4 + contacts integration +1・全体で unit 444 / integration 275 緑
- メール送信の多元化＝Gmail 送信 + 汎用 SMTP（2026-07-24・オーナー「メール機能は resend や sendgrid に限られますか？」→「1,3 は実装したい」）— 実装済み: ①**本人の Gmail から個別メールを送る** = `GOOGLE_SCOPES_MAIL_SEND`（gmail.send・制限付き区分・設定ページの明示オプトイン `auth-url?scope=mailsend`・status に mailSend）。`gmailMailerFor(ownerUid)` が接続の許可を見て **個別 outreach（send・予約送信の process-queue）は Gmail を配信サービスより優先**（相手には本人のメールとして届き返信も本人の Gmail に残る。**経路は決定的に 1 本＝失敗時のフォールバック連鎖はしない**）。送信は `buildGmailRaw` 純粋関数（RFC822/RFC2047・base64url）+ messages.send、トークンは gmail.send 単独にダウンスコープ。**一斉配信・提携先メールは従来どおり配信サービスのみ**（Gmail の送信上限と役割分担）。②**汎用 SMTP** = `SMTP_URL`（例 smtps://user:pass@host:465・nodemailer・番兵値 unset は無効）を設定すると Resend/SendGrid より最優先。Secret `BONDS_SMTP_URL` 存在時のみ配線。mailer-status に provider=smtp と gmailSendOwners を追加。オーナー手順は OWNER-SETUP タスク1-E（Gmail・設定ページのボタン 1 分）/1-F（SMTP）。テスト: mailer unit +2（SMTP 優先・buildGmailRaw の RFC2047/復号）+ outreach integration +1（mailer 無しでも Gmail 許可があれば送れる・ダウンスコープ・providerMessageId）
