...
- フェーズ5: 認証三段フェイルセーフ・監査ログ・本番 Dockerfile・デプロイスクリプト（cares 同一プロジェクト / ANTHROPIC_API_KEY Secret 共有）— 実装済み（GCP 実機での初回デプロイ・web ログイン UI・多言語 UI 辞書・一括配信キュー・ライブカレンダー同期は残）
